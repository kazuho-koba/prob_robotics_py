...
# sec5.5.pyの内容を外部から参照できるようにしたコード。
# 粒子の中から最も良さそうなものをその時点の自己位置推定結果として提供する。
...
class Mcl:
    ...
    def observation_update(self, observation):
        ...
        # 粒子のうちもっともロボット姿勢として尤度が高そうなものを選定
        self.set_ml()
        # 粒子をリサンプリング（尤度が低いものが復活することはないので消してしまい、その分尤度が高い粒子をその周辺に分割して増やしてやる）
        self.resampling()

    # 粒子をリサンプリングするメソッド
    def resampling(self):
        ### 通常のやり方：計算量がO(NlogN）必要で、サンプリングバイアスもある（どこに？）####
        # ws = [e.weight for e in self.particles]     # 粒子の重みのリスト
        # if sum(ws) < 1e-100:
        #     ws = [e + 1e-100 for e in ws]          # 重みがゼロになるとエラーを吐くので小さな値を足す
        # ps = random.choices(self.particles, weights=ws, k=len(self.particles))  # 粒子リストから重みに比例した確率でk個（今回の場合は粒子総数）選ぶ
        # # 選んだリストから粒子を取り出して重みを均一にする
        # self.particles = [copy.deepcopy(e) for e in ps]                         
        # for p in self.particles:
        #     p.weight = 1.0/len(self.particles)

        ### 改善したやり方：計算量がO(N)で、サンプリングバイアスも解消されている
        ws = np.cumsum([e.weight for e in self.particles])      # 重みの累積和（最後の要素＝重みの総和）
        if ws[-1] < 1e-100:
            ws = [e + 1e-100 for e in ws]                       # 重みの総和が限りなくゼロに近いときの処理
